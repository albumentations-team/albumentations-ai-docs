...
Albumentations 2.0.19+ simplifies symmetry-based TTA: transforms like [`D4`](https://explore.albumentations.ai/transform/D4) and [`SquareSymmetry`](https://explore.albumentations.ai/transform/SquareSymmetry) support a `group_element` argument for deterministic enumeration, and spatial transforms provide `.inverse()` to map predictions back to the original coordinate system. The removal of strict memory contiguity requirements also improves TTA throughput.
...
def horizontal_flip_tta(model, image, preprocess):
    ...
    # Average logits
    with torch.no_grad():
        logits_orig = model(orig.unsqueeze(0).to(device))
        logits_flip = model(flip_processed.unsqueeze(0).to(device))
...
![TTA equivariance schema for semantic segmentation](../../img/advanced/tta_equivariance_segmentation.webp)

Albumentations 2.0.19+ provides `.inverse()` on spatial transforms ([`HorizontalFlip`](https://explore.albumentations.ai/transform/HorizontalFlip), [`VerticalFlip`](https://explore.albumentations.ai/transform/VerticalFlip), [`D4`](https://explore.albumentations.ai/transform/D4), etc.), eliminating manual inversion logic:
...
def segmentation_hflip_tta(model, image, preprocess):
    """TTA for segmentation with HorizontalFlip using inverse()."""
    device = next(model.parameters()).device
    aug = A.HorizontalFlip(p=1.0)

    # Original prediction
    input_orig = preprocess(image=image)["image"].unsqueeze(0).to(device)
    with torch.no_grad():
        pred_orig = model(input_orig)  # [1, C, H, W]

    # Flipped prediction
    flipped = aug(image=image)["image"]
    input_flip = preprocess(image=flipped)["image"].unsqueeze(0).to(device)
    with torch.no_grad():
        pred_flip = model(input_flip)  # [1, C, H, W]

    # Map prediction back using inverse() — convert to numpy for Albumentations
    pred_flip_np = pred_flip.squeeze(0).permute(1, 2, 0).cpu().numpy()
    pred_flip_aligned_np = aug.inverse()(image=pred_flip_np)["image"]
    pred_flip_aligned = torch.from_numpy(pred_flip_aligned_np).permute(2, 0, 1).unsqueeze(0).to(device)

    avg_prediction = (pred_orig + pred_flip_aligned) / 2
    return avg_prediction
```

For numpy-based pipelines (e.g., model outputs `(H, W, C)`), the inverse step is simpler:

```python
aug = A.HorizontalFlip(p=1.0)
pred_flip_aligned = aug.inverse()(image=prediction_numpy)["image"]
```

> [!IMPORTANT]
> The critical step that distinguishes segmentation TTA from classification TTA is applying the **inverse geometric transform** to the prediction map before averaging. Use `.inverse()` (2.0.19+) on [`HorizontalFlip`](https://explore.albumentations.ai/transform/HorizontalFlip), [`VerticalFlip`](https://explore.albumentations.ai/transform/VerticalFlip), [`D4`](https://explore.albumentations.ai/transform/D4), [`SquareSymmetry`](https://explore.albumentations.ai/transform/SquareSymmetry), [`RandomRotate90`](https://explore.albumentations.ai/transform/RandomRotate90), and [`Transpose`](https://explore.albumentations.ai/transform/Transpose) instead of manual inversion. Color transforms (brightness, contrast) don't have spatial inverses — they only affect the input, not the output map.
...
In Albumentations, this is captured by [`D4`](https://explore.albumentations.ai/transform/D4) and [`SquareSymmetry`](https://explore.albumentations.ai/transform/SquareSymmetry). As of 2.0.19, these transforms support a `group_element` argument for deterministic enumeration and an `.inverse()` method to map predictions back to the original coordinate system.

#### D4-based TTA Example (2.0.19+)

The following spatial transforms provide `.inverse()` for mapping model outputs back to the original coordinate system: [`D4`](https://explore.albumentations.ai/transform/D4), [`SquareSymmetry`](https://explore.albumentations.ai/transform/SquareSymmetry), [`HorizontalFlip`](https://explore.albumentations.ai/transform/HorizontalFlip), [`VerticalFlip`](https://explore.albumentations.ai/transform/VerticalFlip), [`RandomRotate90`](https://explore.albumentations.ai/transform/RandomRotate90), [`Transpose`](https://explore.albumentations.ai/transform/Transpose).
...
final_prediction = np.stack(predictions).mean(axis=0)
```
...
![TTA Accuracy vs Number of Views](../../img/advanced/tta_accuracy_plot.webp)
...
5. **For spatial tasks** (segmentation, detection), apply **inverse transforms** to predictions — use `.inverse()` (2.0.19+) on supported transforms instead of manual logic
6. **For D4 symmetry**, use `group_element` with `d4_group_elements` for deterministic enumeration
7. **Benchmark the improvement** — measure accuracy with and without TTA to confirm it helps for your specific model and dataset
